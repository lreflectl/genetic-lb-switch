...
def install_ports_to_path(path: list[int], src_port: int, dst_port: int, link_tree: dict[int, dict[int, dict]]) \
    ...
    """ Install corresponding ports to every switch in the path. As link tree already has
        best port among every two switches selected by length, the path is optimal. """
    ported_path = []
    ...
    print('path =', path)
    first_sw = (path[0], src_port, link_tree[path[0]][path[1]]['port'])
    ported_path.append(first_sw)
    for idx in range(1, len(path) - 1):
        prev_sw = path[idx-1]
        curr_sw = path[idx]
        next_sw = path[idx+1]
        ported_sw = (curr_sw, link_tree[curr_sw][prev_sw]['port'], link_tree[curr_sw][next_sw]['port'])
        ported_path.append(ported_sw)
    last_sw = (path[-1], link_tree[path[-1]][path[-2]]['port'], dst_port)
